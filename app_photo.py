...
class CameraApp:
    def __init__(self) -> None:
        ...
        # Keep preview stable by default: Auto AF (one-shot capable) instead of continuous hunting.
        self.af_mode = int(os.getenv("PIMAGE_AF_MODE", "1"))
        if self.af_mode not in {0, 1, 2}:
            self.af_mode = 1
    ...
    def buttons(self):
        ...
        def make_row(row_actions, y):
            ...
            n = len(row_actions)
            gap = 6
            # Fit width strictly inside the safe area.
            bw = int((usable_w - gap * (n - 1)) / n)
            bw = max(64, bw)
            total = bw * n + gap * (n - 1)
            start_x = max(side_margin, (self.screen_w - total) // 2)
